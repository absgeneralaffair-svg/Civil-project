"use client";
import { Layers, List, Percent, AlertTriangle, Clock, CheckCircle2, PlayCircle, AlertCircle, X, FileSpreadsheet, Boxes } from "lucide-react";
...
export default function DashboardTab({ projects, subPekerjaan, rabs, materials, penggunaanLogs, loading, selectedYear, allData }) {
  const [modalData, setModalData] = useState(null);
  
  // Kalkulasi Stok Barang dengan MR, PR, Peruntukan
  const getMaterialDetails = (mat) => {
    if (!allData) return { ...mat, mrList: "-", prList: "-", peruntukan: "-" };
    const bMasuk = (allData.barangMasuk || []).filter(bm => bm.materialId === mat.id);
    const relatedOrders = (allData.ordersData || []).filter(o => 
      bMasuk.some(bm => bm.orderId === o.id) || 
      (mat.kode && mat.kode.trim() !== "-" && mat.kode.trim() !== "" && o.partnumber === mat.kode) || 
      (mat.nama && mat.nama.trim() !== "-" && mat.nama.trim() !== "" && o.nama === mat.nama)
    );

    const mrs = [...new Set([...bMasuk.map(b => b.mr), ...relatedOrders.map(o => o.mr)].filter(Boolean))];
    const prs = [...new Set([...bMasuk.map(b => b.pr), ...relatedOrders.map(o => o.pr)].filter(Boolean))];
    const keperluans = [...new Set([...bMasuk.map(b => b.keperluan), ...relatedOrders.map(o => o.keperluan), ...(allData.penggunaan || []).filter(p => p.materialId === mat.id).map(p => p.keterangan)].filter(Boolean))];

    return {
       ...mat,
       mrList: mrs.length ? mrs.join(", ") : "-",
       prList: prs.length ? prs.join(", ") : "-",
       peruntukan: keperluans.length ? keperluans.join(", ") : "-"
    };
  };

  const stokBarangDetailed = materials.filter(mat => (Number(mat.stok) || 0) > 0).map(getMaterialDetails);
  ...
  const handleExportExcel = () => {
    const columns = [
      { header: "Fase Pekerjaan", key: "nama", width: 30 },
      { header: "Jadwal Mulai", key: "mulai", width: 15 },
      { header: "Jadwal Selesai", key: "selesai", width: 15 },
      { header: "Target (%)", key: "target", width: 10 },
      { header: "Realisasi (%)", key: "progres", width: 10 },
      { header: "Status Jadwal", accessor: (row) => row.schedule.statusText, width: 25 },
    ];
    exportToExcel(analyzedProjects, columns, "Dashboard_Projects_Data");
  };
  ...
  return (
    <section id="tab-dashboard" className="tab-panel active" style={{ display: 'block', animation: 'fadeIn 0.3s ease', overflowY: 'auto', paddingRight: '10px' }}>
      ...
      {/* ACTION BAR */}
      <div className="action-bar" style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '15px' }}>
        <button className="btn btn-secondary" onClick={handleExportExcel} title="Export ringkasan dashboard ke Excel">
          <FileSpreadsheet size={16} style={{ marginRight: '8px' }} /> Export Ringkasan ke Excel
        </button>
      </div>
      ...
      <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', marginBottom: '24px' }}>
        ...
        <div className="kpi-card glow-green" onClick={() => setModalData({ title: "Detail Stok Barang (MR/PR & Peruntukan)", type: "stok_barang", data: stokBarangDetailed })} style={{ cursor: 'pointer', transition: 'transform 0.2s' }}>
          <div className="kpi-icon-box bg-green"><Boxes size={20} /></div>
          <div className="kpi-info">
            <span className="kpi-label">Stok Barang</span>
            <h3 className="kpi-value">{loading ? "..." : stokBarangDetailed.length} Item</h3>
            <span className="kpi-trend trend-green" style={{ fontSize: '0.75rem', marginTop: '4px', display: 'block' }}>Klik untuk detail material</span>
          </div>
        </div>
      ...
      <div className="dashboard-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
        ...
        <div className="glass-card" style={{ borderTop: "4px solid var(--accent-green)" }}>
          <div className="card-header" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Boxes size={22} color="var(--accent-green)" />
            <div>
              <h4 className="card-title">Stok Barang</h4>
              <p className="card-subtitle">Daftar stok material gudang beserta informasi MR/PR.</p>
            </div>
          </div>
          <div className="table-container" style={{ marginTop: '16px', maxHeight: '350px', overflowY: 'auto' }}>
            <table className="table" style={{ minWidth: "100%" }}>
              ...
              <tbody>
                ...
                ) : stokBarangDetailed.length === 0 ? (
                  <tr><td colSpan="3" style={{ textAlign: "center", color: "var(--text-muted)", padding: "20px" }}>Belum ada stok barang.</td></tr>
                ) : (
                  stokBarangDetailed.map(mat => (
                    <tr key={mat.id} onClick={() => setModalData({ title: "Detail Stok Barang (MR/PR & Peruntukan)", type: "stok_barang", data: stokBarangDetailed })} style={{ cursor: 'pointer' }}>
                      <td style={{ fontWeight: 600, color: "var(--accent-green)" }}>{mat.nama}</td>
                      <td>{mat.stok} {mat.satuan}</td>
                      <td style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>
                        <div>MR: {mat.mrList}</div>
                        <div>PR: {mat.prList}</div>
                      </td>
                    </tr>
                  ))
                )}
        ...
        <div className="modal active" style={{ zIndex: 9999 }}>
          <div className="modal-content glass-card" style={{ maxWidth: '800px' }}>
            ...
            <div className="modal-body table-container" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
              {modalData.type === "stok_barang_detail" ? (
                <div>
                  <button className="btn btn-secondary" style={{ marginBottom: "16px", display: "flex", alignItems: "center", gap: "8px" }} onClick={() => setModalData({ title: "Detail Stok Barang (MR/PR & Peruntukan)", type: "stok_barang", data: materials.filter(mat => (Number(mat.stok) || 0) > 0).map(m => {
                    const bMasuk = (allData.barangMasuk || []).filter(bm => bm.materialId === m.id);
                    const relatedOrders = (allData.ordersData || []).filter(o => 
                      bMasuk.some(bm => bm.orderId === o.id) || 
                      (m.kode && m.kode.trim() !== "-" && m.kode.trim() !== "" && o.partnumber === m.kode) || 
                      (m.nama && m.nama.trim() !== "-" && m.nama.trim() !== "" && o.nama === m.nama)
                    );
                    const mrs = [...new Set([...bMasuk.map(b => b.mr), ...relatedOrders.map(o => o.mr)].filter(Boolean))];
                    const prs = [...new Set([...bMasuk.map(b => b.pr), ...relatedOrders.map(o => o.pr)].filter(Boolean))];
                    const keperluans = [...new Set([...bMasuk.map(b => b.keperluan), ...relatedOrders.map(o => o.keperluan), ...(allData.penggunaan || []).filter(p => p.materialId === m.id).map(p => p.keterangan)].filter(Boolean))];
                    return { ...m, mrList: mrs.length ? mrs.join(", ") : "-", prList: prs.length ? prs.join(", ") : "-", peruntukan: keperluans.length ? keperluans.join(", ") : "-" };
                  }) })}>
                  ...
                  <table className="table" style={{ minWidth: "100%" }}>
                    ...
                    <tbody>
                      {modalData.data.length === 0 ? (
                        <tr><td colSpan="6" style={{ textAlign: "center", padding: "30px", color: "var(--text-muted)" }}>Tidak ada rincian penerimaan</td></tr>
                      ) : (
                        modalData.data.map(log => (
                          <tr key={log.id}>
                            <td style={{ fontWeight: 600, color: "var(--accent-blue)" }}>{modalData.matInfo.nama}</td>
                            <td>{log.tanggal}</td>
                            <td style={{ fontWeight: 600 }}>{log.jumlah} {modalData.matInfo.satuan}</td>
                            <td>{log.mr || '-'}</td>
                            <td>{log.pr || '-'}</td>
                            <td>{log.keperluan || '-'}</td>
                          </tr>
                ...
                <table className="table" style={{ minWidth: "100%" }}>
                  ...
                  <tbody>
                      {modalData.data.length === 0 ? (
                        <tr><td colSpan="6" style={{ textAlign: "center", padding: "30px", color: "var(--text-muted)" }}>Tidak ada data</td></tr>
                      ) : (
                        modalData.data.map(mat => (
                          <tr key={mat.id} onClick={() => {
                            const bMasuk = (allData.barangMasuk || []).filter(bm => bm.materialId === mat.id);
                            setModalData({
                                title: `Rincian Material: ${mat.nama}`,
                                type: "stok_barang_detail",
                                data: bMasuk,
                                matInfo: mat
                            });
                          }} style={{ cursor: "pointer" }} className="hover-row">
                            <td>{mat.kode}</td>
                            <td style={{ fontWeight: 600, color: "var(--accent-blue)" }}>{mat.nama}</td>
                            <td style={{ fontWeight: 600 }}>{mat.stok} {mat.satuan}</td>
                            <td>{mat.mrList}</td>
                            <td>{mat.prList}</td>
                            <td>{mat.peruntukan}</td>
                          </tr>
                        ))
                      )}
                  </tbody>

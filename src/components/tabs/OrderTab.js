      ...
      <div className="glass-card">
        <div className="action-bar" style={{ justifyContent: 'space-between', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <button className="btn btn-secondary" title="Export data ke Excel (xls)">
              ...
              {loading ? (
                ...
                processedOrders.map(item => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span className={`badge badge-${item.calcStatus === 'Selesai' ? 'green' : item.calcStatus === 'Partial' ? 'blue' : 'orange'}`}>
                        {item.calcStatus}
                      </span>
